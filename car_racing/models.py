...
class RSSM_NA(tools.Module):

    def __init__(self, stoch=30, deter=200, hidden=200, act=tf.nn.elu, normalize=False):
        super().__init__()
        self._activation = act
        self._stoch_size = stoch
        self._deter_size = deter
    ...
    def observe(self, embed, action, state=None):
        ...
        prior = {k: tf.transpose(v, [1, 0, 2]) for k, v in prior.items()}
        if self._normalize:
            post = {k: tf.math.l2_normalize(v, -1) for k, v in post.items()}
            prior = {k: tf.math.l2_normalize(v, -1) for k, v in prior.items()}
        return post, prior
    ...
    def get_feat(self, state):
        if self._normalize:
            return tf.math.l2_normalize(tf.concat([state['stoch'], state['deter']], -1), -1)
        return tf.concat([state['stoch'], state['deter']], -1)
    ...
    @tf.function
    def obs_step(self, prev_state, prev_action, embed=None):
        prior = self.img_step(prev_state, prev_action)
        x = tf.concat([prior['deter'], embed], -1)
...
class ConvEncoderProprioLarge(tools.Module):
    ...
    def __call__(self, obs):
        ...
        embed = tf.reshape(x, shape)
        embed = tf.concat([embed, obs['proprio']], axis = -1)
        return embed
...
class ConvDecoderMaskLarge(tools.Module):
    ...
    def __call__(self, features):
        ...
        x = self.get('h6', tfkl.Conv2DTranspose, 3 + self._shape[-1], 6, strides=2)(x)
        mean, mask = tf.split(x, [3, 3], -1)
        mean = tf.reshape(mean, tf.concat(
            [tf.shape(features)[:-1], self._shape], 0))
        mask = tf.reshape(mask, tf.concat(
