...
class CarRacing:
    ...
    def step(self, action):
        ...
        obs = {'image': image}
        obs['image'] = self._noisy_obs(obs['image'])
    ...
    def _noisy_obs(self, obs):
      obs = obs[:80]
      obs = cv2.resize(obs, self.shape2d)
      if self._img_source is not None:
        mask = np.logical_and((obs[:, :, 1] > obs[:, :, 2]), (obs[:, :, 1] > obs[:, :, 0]))  # hardcoded for dmc
