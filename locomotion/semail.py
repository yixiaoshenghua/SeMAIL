...
os.environ['MUJOCO_GL'] = 'osmesa' # glfw, egl
...
os.environ["CUDA_VISIBLE_DEVICES"] = str(tools.get_device(memory_limits=13000))
...
class SeMAIL(tools.Module):
    ...
    def _train(self, data, expert_data, log_images):
        with tf.GradientTape(persistent=True) as model_tape:
            # model_data {'image': (bs, t, 84, 84, 3), 'action': (bs, t, 6), 'reward': (bs, t), 'discount': (bs, t)}
            # main agent
        ...
        with tf.GradientTape(persistent=True) as agent_tape:
            ...
            with tf.GradientTape() as penalty_tape: # WGAIL-GP
    ...
    def _build_model(self):
        ...
        self._encode = models.ConvEncoder(self._c.cnn_depth, cnn_act, self._c.image_size)
        ...
        disen_modules = [self._disen_encode, self._disen_dynamics, self._disen_only_decode]#, self._exp_disen_reward]
...
def make_env(config, writer, prefix, model_datadir, policy_datadir, video_dir, store):
...
def main(config):
    ...
    if config.copy_datadir is not None:
        from distutils.dir_util import copy_tree
        copy_tree(str(config.copy_datadir), str(config.model_datadir))
    else:
        from distutils.dir_util import copy_tree
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='dmc_walker_run_driving-driving')
    parser.add_argument('--video_datadir', type=str, default='../driving_car_16/driving_car_16')
    parser.add_argument('--expert_dir', type=str, default='../data/walker_run_expert')
    parser.add_argument('--seed', type=int, default=2021)
    parser.add_argument('--steps', type=int, default=1000000)
    parser.add_argument('--image_size', type=int, default=64)
    parser.add_argument('--disen_rec_scale', type=float, default=0.25)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--copy_datadir', default=None, type=str)
    ...
    tools.seed_tensorflow(args.seed)
